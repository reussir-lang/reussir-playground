//! Rust harness generation for the wasm32-wasip1 "run" mode.
//!
//! The generated `main.rs` is compiled as a wasm32-wasip1 binary.  It:
//!  1. Declares the FFI trampoline symbols exported by the compiled Reussir
//!     object (via `extern "C" { … }`).
//!  2. Links `reussir-rt` (via `extern crate reussir_rt`) to pull in the
//!     `__reussir_allocate` / `__reussir_deallocate` / … symbols.
//!  3. Runs the user-supplied driver inside `main`.
//!
//! The server never executes this wasm binary — it ships it to the browser,
//! which runs it through the bundled WASI shim.
...
/// Generate the `src/main.rs` for the throwaway wasm harness crate.
///
/// * `source` — the full Reussir source text (to extract function signatures
///              and trampoline declarations).
/// * `driver` — Rust code embedded verbatim inside an `unsafe` block in
///              `main`.
pub fn generate_harness(source: &str, driver: &str) -> Result<String, String> {
    ...
    for t in &trampolines {
        let func = functions
            ...
            .ok_or_else(|| {
                format!(
                    "function '{}' referenced by trampoline '{}' not found in source",
    ...
    re.captures_iter(source)
        .map(|cap| {
            ...
            FuncSig {
                ...
                type_params,
                params,
                ret: cap.get(4).map(|m| m.as_str().to_string()),
            }
        })
        .collect()
}
...
fn generate_ffi_decl(trampoline: &Trampoline, func: &FuncSig) -> Result<String, String> {
    ...
    let params: Vec<String> = func
        .params
        .iter()
        .map(|(name, ty)| format!("{name}: {}", resolve_type(ty, &type_map)))
        .collect();

    let ret_str = func
        .ret
        .as_deref()
        .map(|ty| format!(" -> {}", resolve_type(ty, &type_map)))
        .unwrap_or_default();
...
fn resolve_type<'a>(ty: &'a str, type_map: &HashMap<&str, &'a str>) -> String {
    let resolved = type_map.get(ty).copied().unwrap_or(ty);
...
"#;
        let harness = generate_harness(source, "").unwrap();
        assert!(harness.contains("fn fibonacci_ffi(n: u64) -> u64;"));
    }
    ...
    fn test_no_trampolines() {
        ...
        let harness = generate_harness(source, driver).unwrap();
        assert!(!harness.contains("unsafe extern \"C\""));
        assert!(harness.contains("extern crate reussir_rt;"));
        assert!(harness.contains(driver));
    }
    ...
    fn test_opaque_pointer_types() {
        let source = r#"
fn make_leaf(v : i32) -> Tree { v }
fn count_leaves(t : Tree) -> i32 { 1 }
